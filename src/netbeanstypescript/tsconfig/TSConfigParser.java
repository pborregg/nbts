...
public class TSConfigParser extends Parser {
    ...
    public void parse(Snapshot snapshot, Task task, SourceModificationEvent sme) throws ParseException {
        // TODO: find out if any locking is necessary
        final TokenSequence<?> ts = snapshot.getTokenHierarchy().tokenSequence();
                ...
                switch ((JsTokenId) ts.token().id()) {
                    ...
                    case BRACKET_LEFT_CURLY:
                        ...
                        for (boolean in = false;
                            ...
                            int colonOffset = ts.offset();
                            advance();
                            int valueStartOffset = ts.offset();
                            ConfigNode value = value();
                            if (value == null) {
                                value = new ConfigNode();
                                value.startOffset = colonOffset + 1;
                                value.endOffset = valueStartOffset;
                            }
                            ...
                            value.keyOffset = keyNode.startOffset;
                            ConfigNode oldValue = node.properties.put(key, value);
                            if (oldValue != null) {
